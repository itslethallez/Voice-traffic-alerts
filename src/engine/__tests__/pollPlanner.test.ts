...
describe('planPoll', () => {
  ...
  it('widens the moving box - ahead and side, proportionally - when the announce distance exceeds it', () => {
    const driver: DriverState = { position: ADELAIDE_POSITION, headingDeg: 45, speedKmh: 60 };
    const movement = updateMovementState(initialMovementState, ADELAIDE_POSITION, 0);
    ...
    expect(plan.boundingBox).toEqual(
      boundingBox(ADELAIDE_POSITION, 45, 20_000, 20_000 * (MOVING_BOX_SIDE_M / MOVING_BOX_AHEAD_M))
    );
  });
