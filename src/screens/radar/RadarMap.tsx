 ...
 * linked (Expo Go, a build that predates this dependency, iOS/web - out
 * of scope for this step). A static top-level `import` would take down
 * the whole JS bundle before this component ever got a chance to render
 * a fallback. Loading it lazily behind a try/catch keeps that throw
 * local to this module and catchable, so everything else in the app -
 * including the rest of this radar-style Drive screen - keeps working.
 */
...
export function RadarMap() {
  ...
  if (Platform.OS !== 'android') {
    return <Unsupported message="Radar map is Android-only for now." />;
  }
  if (!Mapbox) {
    return (
