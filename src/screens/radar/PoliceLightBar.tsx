...
const OFF_BLUE = alpha(colors.coolBlue, 0.18);
const OFF_RED = alpha(colors.critical, 0.18);
...
export function PoliceLightBar({
  ...
  const offBlue = inverted ? colors.charcoal : OFF_BLUE;
  const offRed = inverted ? colors.charcoal : OFF_RED;
  ...
  const blueCellStyle = useAnimatedStyle(() => ({
    backgroundColor: interpolateColor(phase.value, [0, 1], [colors.coolBlue, offBlue]),
    shadowOpacity: interpolate(phase.value, [0, 1], [0.85, 0]),
  }));
  const redCellStyle = useAnimatedStyle(() => ({
    backgroundColor: interpolateColor(phase.value, [0, 1], [offRed, colors.critical]),
