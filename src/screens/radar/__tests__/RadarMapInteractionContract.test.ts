...
describe('RadarMap interaction and marker contract', () => {
  ...
  it('keeps the web adapter aligned for focus sequencing, labels, and flat range mode', () => {
    ...
    expect(webSource).toContain('NAVIGATING_PITCH = 62');
    expect(webSource).toContain('CRUISING_PITCH = 50');
    expect(webSource).toContain('pitch: NAVIGATING_PITCH');
    expect(webSource).toContain('pitch: isNavigating ? NAVIGATING_PITCH : CRUISING_PITCH');
